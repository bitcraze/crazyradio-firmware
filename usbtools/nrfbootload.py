...
try:
    if action == "dump":
        ...
        print(f"{length} bytes written in {filename}.")
        ...
        print(f"Reading {length} bytes of the flash from address 0x{address:04X}")
        flash = bl.read(address, length)
        ...
        for i in range(0, len(flash)):
            if i % 16 == 0:
                if i != 0:
                    print()
                sys.stdout.write(f"0x{address+i:04X}: ")
            if i % 8 == 0 and i % 16 != 0:
                sys.stdout.write(" ")
            sys.stdout.write("{:02X}".format(flash[i]))
    ...
    if action == "flash":
        ...
        print(f"  Flashing {len(fileData)} bytes...")
    ...
    if action == "verify" or action == "flash":
        ...
        print(f"  Reading {len(fileData)} bytes from the flash...")
        flashData = bl.read(0, len(fileData))

        fail = False
        for i in range(0, len(fileData)):
            if fileData[i] != flashData[i]:
                print(f"Verification failed at byte {i} (expected 0x{fileData[i]:02X}, read 0x{flashData[i]:02X})!")
                fail = True
                break
